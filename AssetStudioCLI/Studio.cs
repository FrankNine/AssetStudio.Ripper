...
internal static class Studio
{
    ...
    private static List<AssetItem> parsedAssetsList = [];
    private static List<BaseNode> gameObjectTree = [];
    ...
    {
        GameData gameData = AssetsManager.LoadPaths(CLIOptions.inputPath);
        if (gameData.GameBundle.Collections.Count == 0)
        {
    ...
    public static void ParseAssets()
    {
        ...
        var objectCount = assetsManager.assetCollectionList.Sum(x => x.Objects.Count);
        ...
        Progress.Reset();
        var i = 0;
        foreach (var assetsFile in assetsManager.assetCollectionList)
        {
        ...
        var log = $"Finished loading {assetsManager.assetCollectionList.Count} files with {parsedAssetsList.Count} exportable assets";
        var unityVer = assetsManager.assetCollectionList[0].version;
        long m_ObjectsCount;
        if (unityVer > 2020)
        {
            m_ObjectsCount = assetsManager.assetCollectionList.Sum(x => x.m_Objects.LongCount(y =>
        ...
        {
            m_ObjectsCount = assetsManager.assetCollectionList.Sum(x => x.m_Objects.LongCount(y => CLIOptions.o_exportAssetTypes.Value.Any(k => (int)k == y.classID)));
        }
        var objectsCount = assetsManager.assetCollectionList.Sum(x => x.Objects.LongCount(y => CLIOptions.o_exportAssetTypes.Value.Any(k => k == y.type)));
    ...
    public static void BuildTreeStructure(Dictionary<AssetStudio.Object, AssetItem> objectAssetItemDic)
    {
        ...
        var assetsFileCount = assetsManager.assetCollectionList.Count;
        int j = 0;
        Progress.Reset();
        foreach (var assetsFile in assetsManager.assetCollectionList)
        {
